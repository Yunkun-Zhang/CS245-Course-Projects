...
class Unit:
    # 初始化
    def __init__(self, fit_fun, dim):
        self.__pos = np.array([np.random.randint(0, 2) for _ in range(dim)])
        self.__mutation = np.array([0 for _ in range(dim)])  # 个体突变后的向量
        self.__crossover = np.array([0 for _ in range(dim)])  # 个体交叉后的向量
...
class DE:
    def __init__(self, fit_fun, dim, size, iter_num, best_fitness_value=float('Inf'), F=0.5, CR=0.8):
        self.F = F
        self.CR = CR
        self.dim = dim  # 维度
        self.size = size  # 总群个数
        ...
        self.best_position = [0 for _ in range(dim)]  # 全局最优解
        self.fitness_val_list = []  # 每次迭代最优适应值
        self.fit_fun = fit_fun

        # 对种群进行初始化
        self.unit_list = [Unit(fit_fun, self.x_min, self.x_max, self.dim) for i in range(self.size)]
    ...
    def selection(self):
        for unit in self.unit_list:
            new_fitness_value = self.fit_fun(unit.get_crossover())
